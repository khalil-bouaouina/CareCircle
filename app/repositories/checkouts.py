"""checkout -- append-only. There is deliberately no update or delete here."""
...
def create_checkout(
    ...
    return execute(
        """INSERT INTO checkout (id, visit_id, completion, observation_codes, note_text, submitted_at)
           VALUES (?, ?, ?, ?, ?, ?)""",
        (
            checkout_id,
            visit_id,
            completion,
            _dump(list(dict.fromkeys(observation_codes))),
            note_text or None,
